...
class CurrentPositionModels {
  ...
  CurrentPositionModels({this.status, this.results});

  factory CurrentPositionModels.fromJson(Map<String, dynamic> parsedJson) {
    var list = parsedJson['results'] as List;
    List<LegalCode> resultLists =
        list.map((i) => LegalCode.fromJson(i)).toList();

    return CurrentPositionModels(
        status: parsedJson['status'], results: resultLists);
  }
}
