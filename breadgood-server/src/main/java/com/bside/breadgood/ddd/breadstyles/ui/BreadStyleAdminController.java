...
public class BreadStyleAdminController {
  ...
  @ApiOperation(value = "관리자에서 최애빵 스타일을 등록합니다.")
  @ApiResponses(value = {
      @ApiResponse(code = 200, message = "성공적으로 등록시 BreadStyleResponseDto 반환", response = BreadStyleResponseDto.class),
      @ApiResponse(code = 400, message = "BadRequest", response = BadRequestError.class),
      @ApiResponse(code = 500, message = "InternalServerError", response = InternalServerError.class),
      @ApiResponse(code = -1, message = "ExceptionResponse", response = ExceptionResponse.class)}
  )
  @PostMapping(value = "", consumes = MediaType.MULTIPART_FORM_DATA_VALUE)
  public BreadStyleResponseDto save(
      @Valid BreadStyleRequestDto requestDto,
      @RequestPart(value = "img") MultipartFile contentImg,
      @RequestPart(value = "profileImg") MultipartFile profileImg
  ) {
    return breadStyleService.save(requestDto, contentImg, profileImg);
  }
}
